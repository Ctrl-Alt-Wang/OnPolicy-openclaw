...
| Run7 | seqkd-run7 | Qwen3-8B | **397B(API)** | - | - | - | - | - | - | **-0.134** |
| Run8 | exp-runs-5-8 | Qwen3-8B | **72B(FP8,è·¨ç³»å)** | **æ ** | 1e-6 | 347 | **100%** | -0.17 | -0.37 | **-0.042** |
...
> Run7/8 full records: [experiment-runs-5-8-record](https://github.com/Ctrl-Alt-Wang/OnPolicy-openclaw/tree/experiment-runs-5-8-record/experiment_logs)
...
## Run7 和 Run8 详细分析（2026-06-13 更新）

### Run7: Seq-KD + Qwen3.5-397B API - 失败

- **日期**: 2026-06-10
- **方法**: Sequential Knowledge Distillation (off-policy)
- **Teacher**: Qwen3.5-397B via SiliconFlow API
- **结果**: avg_delta = **-0.134**（比 baseline 更差）

**失败原因**:
1. API 不支持 logprobs（echo=True 报 500），无法做真正的 OPD
2. 跨系列 tokenizer 不匹配: Qwen3.5 vocab_size=**248320**, Qwen3-8B vocab_size=**151936**
3. Seq-KD 本质是 off-policy 的，样本分布不匹配导致负迁移

---

### Run8: OPD No-Judge + Qwen2.5-72B FP8 - 失败

- **日期**: 2026-06-11 ~ 06-13
- **方法**: OPD 无 judge（与 Run6 相同方法），但用更大的 teacher
- **Teacher**: Qwen2.5-72B-Instruct（本地, FP8 在线量化, SGLang, TP=2）
  - 模型路径: /model/ModelScope/Qwen/Qwen2.5-72B-Instruct
  - SGLang args: --quantization fp8 --mem-fraction-static 0.55 --context-length 2048 --tensor-parallel-size 2
- **Student**: Qwen3-8B（4-bit QLoRA, r=32, lr=1e-6, batch_size=16）
- **训练**: 347 steps, avg_loss = -0.3669, 5552 samples, checkpoint at step_345
- **结果**: avg_delta = **-0.042**（1 正向, 9 负向, 190 neutral）

**失败根因: Tokenizer 词汇表不匹配**

| 模型 | vocab_size | 系列 | 与 Qwen3-8B 兼容？ |
|------|-----------|------|-------------------|
| Qwen3-8B (student) | **151936** | Qwen3 | - |
| Qwen3-32B (Run6 teacher) | **151936** | Qwen3 | 匹配 |
| Qwen2.5-72B (Run8 teacher) | **152064** | Qwen2.5 | 不匹配(差128) |
| Qwen3.5-9B | **248320** | Qwen3.5 | 不匹配(差巨大) |

OPD 的核心是计算 teacher 和 student 在**相同 token** 上的 KL 散度。当 vocab 不匹配时:
- Teacher 产生的 token ID 151936~152063 在 student 词表中不存在
- KL 散度在**不对齐的分布**上计算 = 训练信号被污染
- 表现为: 初始 loss 极低(-0.17 vs Run6 的 -0.43), 最终 loss 量级也低(-0.37 vs -0.63)
...
**关键对比**: Run6 avg_loss 是 Run8 的 1.7 倍 (-0.63 vs -0.37)，同系列 teacher 提供了更强更有效的 KL 散度信号。
...
| 时间 | 事件 |
|------|------|
| 06-10 14:05 | 用户要求分析先前对话历史 |
| 06-10 15:00 | 决策: Run8 用更强 teacher |
| 06-10 15:02 | 发现服务器仅 2 卡, 只能跑 72B 而非 235B |
| 06-11 00:05 | 确认方案: Run8 = OPD + Qwen2.5-72B FP8 |
| 06-11 00:07 | 通过 paramiko SSH 连接服务器 |
| 06-11 00:26~01:36 | 下载模型 (ModelScope -> HuggingFace 多次失败) |
| 06-11 01:38 | **讨论了 vocab 一致性但未实际验证** (关键疏漏!) |
| 06-11 01:45 | 确认无 judge (真正 OPD), 启动训练 |
| 06-11 02:48 | Step 1 完成, GPU 正常, 训练稳定 |
| 06-12 09:00 | 训练完成 (347 steps), 开始评测 |
| 06-12 16:20 | **Claude Code 订阅过期**, 后续无法响应 |
| 06-13 12:45 | 通过 Codex CLI 重新评分, 200/200 完成 |

---

### 核心教训

1. **Tokenizer 兼容性是不可妥协的前提** - 训练前必须验证 vocab_size 一致
2. **更大 teacher 不必然更好** - tokenizer 不匹配时 72B 远不如 32B
3. **Loss 曲线是重要诊断信号** - 初始 loss 过低 = KL 散度信号异常
4. **No-Judge OPD > Judge OPD** (Run6 vs Run5 再次验证)
5. **Seq-KD 对此任务不可行** - off-policy + tokenizer 不匹配
6. **过程管理教训**: vocab 检查被讨论但未执行; Claude Code 订阅中途过期导致监控中断; 评分 daemon 脚本有 shell bug

---

### 下一步: Run9

**Run9: OPD + Qwen3-72B (同系列 Teacher)** - 最高优先级
- 服务器 /workspace/Qwen3-72B 下载未完成 (仅 32KB), 需先完成下载
- Qwen3-72B vocab_size 应为 151936 (与 Qwen3-8B 完全一致)
- FP8 量化可在 2x A800 运行 (72B FP8 ~72GB, TP=2)
- **这是验证"更强同系列 teacher 是否有效"的唯一干净对照实验**
- 训练前必须确认 vocab_size = 151936

---

> 完整实验数据、脚本、对话记录: [experiment_logs/](experiment_logs/) | [完整分析报告](experiment_logs/EXPERIMENT_REPORT.md)
