...
## Run7 ? Run8 ?????2026-06-13 ???

### Run7: Seq-KD + Qwen3.5-397B API - ??

- **??**: 2026-06-10
- **??**: Sequential Knowledge Distillation (off-policy)
- **Teacher**: Qwen3.5-397B via SiliconFlow API
- **??**: avg_delta = **-0.134**?? baseline ???

**????**:
1. API ??? logprobs?echo=True ? 500???????? OPD
2. ??? tokenizer ???: Qwen3.5 vocab_size=**248320**, Qwen3-8B vocab_size=**151936**
3. Seq-KD ??? off-policy ??????????????

---

### Run8: OPD No-Judge + Qwen2.5-72B FP8 - ??

- **??**: 2026-06-11 ~ 06-13
- **??**: OPD ? judge?? Run6 ??????????? teacher
- **??**: avg_delta = **-0.042**?1 ??, 9 ??, 190 neutral?

#### ??????

**1. ????**?06-10?

Run6 ? Qwen3-32B teacher ?? +0.130??????????? teacher ????????

????? Qwen3-235B????????? 2x A800 80GB?????? 235B????320GB?????????**Qwen2.5-72B-Instruct FP8**?2 ? TP=2?

????????? `/model/ModelScope/Qwen/Qwen2.5-72B-Instruct`???????????????? Qwen3-72B ????????????????????? Qwen2.5-72B-Instruct?
...
| ?? | ?? | ?? |
|------|------|------|
| 1 | ModelScope SDK (ms-eb5ae1f1 token) | API ?? 404?SDK ???????? |
| 2 | HuggingFace (hf-mirror.com) | ???????????? token |
| 3 | ??????? HF token | Qwen3-72B ? gated model??????? Accept ?? |
| 4 | ???? HF token + Accept ?? | ?????? download_72b.py????? |
| 5 | ?????? | ?? /workspace/Qwen3-72B ?? 32KB?????? |

???? Qwen3-72B ???????????? **Qwen2.5-72B-Instruct**??? `/model/ModelScope/Qwen/Qwen2.5-72B-Instruct`??????? vocab_size=152064?? Qwen3-8B ? 151936 **???**????????????**?????????**???????????
...
| ?? | ?? | ?? | ?? |
|------|------|------|------|
| 1 | FP8, TP=2, triton backend | ?? | triton backend ? TP=2+FP8 ??? |
| 2 | BF16, TP=2, mem-fraction=0.82 | OOM ?? | 72B BF16 ? 72GB/??0.82x80=65.6GB ?? |
| 3 | BF16, TP=2 (????) | OOM | BF16 ? 2 ??????? |
| 4 | **FP8, TP=2, flashinfer backend, mem-fraction=0.55** | **??** | FP8 ????????70GB???? 44GB/? |

**??????**?SGLang ? `--quantization fp8` ?**??????**?`per_token_group_quant_fp8`????????? BF16 ???????????? = FP8 ??(36GB) + ??? BF16(<1GB) ??? 37GB?????? BF16 ? 72GB??? `--mem-fraction-static 0.55`??? 44GB/GPU??GPU0 ?????32GB ? student ???
...
- `device_map="cuda:0"`?? SGLang TP0 ?? GPU0
- `no_judge=True`???? OPD?? judge ??
- GPU0: ?70.4GB / 80GB?SGLang 47.7GB + student ?? 22.7GB?
- GPU1: ?50.2GB / 80GB?SGLang TP1?

?????347 steps, avg_loss = -0.3669, 5552 samples???? 6 ???
...
- ???200 ???????? 3 ????????
- ??????? `wait_score_run8.sh` ????????????????? GPT-5.4 ??
- **Bug**: ??? shell ?????`wc -l` ??????????????????????????
- ????????? 155/200 ??Claude Code ??????? Claude Code **????**
- ???? Codex CLI + paramiko ???????200/200 ??
...
| Qwen3-32B (Run6 teacher) | **151936** | Qwen3 | ?? |
| **Qwen2.5-72B (Run8 teacher)** | **152064** | Qwen2.5 | **???(?128)** |
| Qwen3.5-9B | **248320** | Qwen3.5 | ???(????) |

OPD ?????? teacher ? student ?**?? token** ?? KL ???
1. Student ? Qwen3-8B tokenizer ?????vocab=151936?
2. Teacher ???? token ???? logprobs
3. ? Qwen2.5-72B ???? 152064 ? token?ID 151936~152063 ? student ????
4. KL ???**??????**??? = ???????
5. ????? loss ??(-0.17 vs Run6 ? -0.43)??? loss ????(-0.37 vs -0.63)
...
**????**: Run6 avg_loss ? Run8 ? 1.7 ? (-0.63 vs -0.37)???? teacher ????????? KL ?????
...
2. **?? teacher ?????** - tokenizer ???? 72B ??? 32B?+0.130 vs -0.042?
3. **Loss ?????????** - ?? loss ?? = KL ???????????????
4. **No-Judge OPD > Judge OPD** (Run6 vs Run5 ????)
5. **Seq-KD ???????** - off-policy + tokenizer ???
6. **????**: vocab ?????????; Claude Code ????????????; ????? shell bug

---

### ???: Run9

**Run9: OPD + Qwen3-72B (??? Teacher)** - ?????
- ??? `/workspace/Qwen3-72B` ????? (? 32KB)???????
- Qwen3-72B vocab_size ?? 151936 (? Qwen3-8B ????)
- FP8 ???? 2xA800 ?? (72B FP8 ?72GB, TP=2)
- **????"????? teacher ????"?????????**
- ???**??**?? vocab_size = 151936

---

> ??????????????: [experiment_logs/](experiment_logs/) | [??????](experiment_logs/EXPERIMENT_REPORT.md)
