...
# CLI 入口
# ---------------------------------------------------------------------------
def _add_output_argument(parser: argparse.ArgumentParser, *, suppress_default: bool = False) -> None:
    kwargs: dict[str, Any] = {"help": "结果输出到 JSON 文件（不指定则输出到 stdout）"}
    if suppress_default:
        kwargs["default"] = argparse.SUPPRESS
    parser.add_argument("--output", "-o", **kwargs)
...
def build_cli_parser() -> argparse.ArgumentParser:
    ...
    _add_output_argument(p_all, suppress_default=True)
    ...
    for name, desc in [
        ...
        _add_output_argument(p, suppress_default=True)
    ...
    _add_output_argument(p_free, suppress_default=True)
