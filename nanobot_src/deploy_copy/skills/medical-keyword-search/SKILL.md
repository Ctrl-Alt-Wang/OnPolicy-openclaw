...
```bash
python <skill_directory>/scripts/medical_search.py all "lung cancer" "immunotherapy" "PD-1" --output /tmp/results.json
```
...
```bash
# 中文指南
python <skill_directory>/scripts/medical_search.py chinese-guideline "diabetes" "insulin" --output /tmp/cn_guide.json

# 英文指南
python <skill_directory>/scripts/medical_search.py english-guideline "hypertension" "ACE inhibitor" --output /tmp/en_guide.json

# 系统评价/Meta分析
python <skill_directory>/scripts/medical_search.py systematic-meta "stroke" "thrombolysis" --output /tmp/meta.json

# RCT
python <skill_directory>/scripts/medical_search.py rct "heart failure" "SGLT2" --output /tmp/rct.json
```

#### 自由搜索（高级用户）

直接传入 InfoX-Med 检索表达式：

```bash
python <skill_directory>/scripts/medical_search.py free \
  --query '(Lung Cancer[Title]) AND (Immunotherapy[Title/Abstract])' \
  --filter '$$doc_publish_time$$2020-01-01$$2025-12-31' \
  --sort docPublishTime \
  --output /tmp/free.json
```

### 步骤 3：读取并展示结果

读取 `--output` 指定的 JSON 文件，整理后向用户展示。
...
```bash
python <skill_directory>/scripts/medical_search.py all "diabetes mellitus" "insulin therapy" "glycemic control" -o /tmp/diabetes.json
```

### 场景 2：查找最新肺癌免疫治疗 RCT

```bash
python <skill_directory>/scripts/medical_search.py rct "lung cancer" "immunotherapy" "PD-1" -o /tmp/lung_rct.json
```
...
```bash
python <skill_directory>/scripts/medical_search.py free \
  --query '(septic shock[Title/Abstract]) AND (fluid resuscitation[Title/Abstract])' \
  --filter '$$doc_publish_time$$2022-01-01$$2025-12-31' \
  --sort docPublishTime \
  -o /tmp/sepsis.json
```
