...
流程:
  1. 发送 /api/science/chat 请求，触发 agent 生成文件
...
# 匹配正文中被替换后的 markdown 下载链接: [filename](/api/science/chat/file?sessionId=xxx&path=xxx)
_DOWNLOAD_LINK_RE = re.compile(
    r'\[([^\]]+)\]\((/api/science/chat/file\?[^)]+)\)'
)
    ...
    if links:
        ...
        if all_md_links:
            ...
            for name, url in all_md_links:
                print(f"[{_ts()}]       - name={name!r}, url={url!r}")
        all_paths = re.findall(r'/api/science/chat/file\?[^\s\)\]]+', full_content)
        if all_paths:
